...
public class ConnectionReflector {
	...
	ConnectionReflector(final ServerManager serverManager,
			final Channel clientConnection) {
		this.parent = serverManager;
		this.clientConnection = clientConnection;
		
	
		/* Add a listener to this channel to stop reflecting if the connection drops. */
		//listenForChannelFailure(clientConnection);
		
		/* add a placeholder handler to consume any bytes that might come through
		 * and to trap any errors that might happen before connection to equator
		 * server can be established.
		 */
		this.clientConnection.pipeline().addLast("byteConsumer", new DiscardHandler());
//	    clientConnection.outboundByteBuffer().writeBytes(new String("HTTP/1.1 200 OK\r\n").getBytes()); //WTF?!?!?!
	}
	...
	public void startReflectingToUIManager(final Channel parmUIManagerChannel) {
		...
		/* listen for channel failure */
//		listenForChannelFailure(uiManagerConnection);
...
//    		
			...
			this.clientConnection.pipeline().addLast("proxyHandler",
					new ProxyHandler(this, this.uiManagerConnection));
			this.uiManagerConnection.pipeline().addLast("proxyHandler",
					new ProxyHandler(this, this.clientConnection));
...
//    private void listenForChannelFailure(Channel channel){
